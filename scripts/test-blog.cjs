...
try {
    for (const name of ['index.html', 'blog/index.html', 'js/site.js', 'data/articles.json', 'scripts/build-blog.cjs']) {
    ...
    for (const name of ['index.html', 'blog/index.html']) {
        ...
        assert.equal((html.match(/<article class="blog-card">/g) || []).length, name === 'index.html' ? 7 : updated.length);
    }
    ...
    console.log('PASS: automatic discovery, newest-first ordering, all three lists, idempotence, existing data and article HTML unchanged.');
} finally {
